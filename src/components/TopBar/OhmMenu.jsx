...
import { ReactComponent as sOhmTokenImg } from "../../assets/tokens/sPAL.svg";
import { ReactComponent as ohmTokenImg } from "../../assets/tokens/PAL.svg";

import "./ohmmenu.scss";
import { usdt, busd, frax } from "src/helpers/AllBonds";
import { useWeb3Context } from "../../hooks/web3Context";

import OhmImg from "src/assets/tokens/PAL.png";
import SOhmImg from "src/assets/tokens/sPAL.png";
import token33tImg from "src/assets/tokens/token_33T.svg";

const addTokenToWallet = (tokenSymbol, tokenAddress) => async () => {
  if (window.ethereum) {
    const host = window.location.origin;
    // NOTE (appleseed): 33T token defaults to sPAL logo since we don't have a 33T logo yet
    let tokenPath;
...
function OhmMenu() {
  ...
  const SPAL_ADDRESS = addresses[networkID].SPAL_ADDRESS;
  const PAL_ADDRESS = addresses[networkID].PAL_ADDRESS;
            ...
            <Fade {...TransitionProps} timeout={100}>
              <Paper className="ohm-menu" elevation={1}>
                <Box component="div" className="buy-tokens">
                  <Link
                    href={`https://pancakeswap.finance/swap?inputCurrency=${busdAddress}&outputCurrency=${PAL_ADDRESS}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                  ...
                  {/* <Link
                    href={`https://swap.spiritswap.finance/#/add/${USDC_ADDRESS}/${PAL_ADDRESS}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <Button size="large" variant="contained" color="secondary" fullWidth>
                  ...
                  {/* <Link href={`https://abracadabra.money/pool/10`} target="_blank" rel="noreferrer">
                    <Button size="large" variant="contained" color="secondary" fullWidth>
                      <Typography align="left">
                        Wrap sPAL on Abracadabra <SvgIcon component={ArrowUpIcon} htmlColor="#A3A3A3" />
                      </Typography>
                    </Button>
                  </Link> */}
                </Box>
                ...
                {/* <Box component="div" className="data-links">
                  ...
                  <Link href={`https://dune.xyz/shadow/Olympus-(PAL)`} target="_blank" rel="noreferrer">
                    <Button size="large" variant="contained" color="secondary" fullWidth>
                      <Typography align="left">
                ...
                {isEthereumAPIAvailable ? (
                  <Box className="add-tokens">
                    ...
                    <Box display="flex" flexDirection="row" justifyContent="space-between">
                      <Button variant="contained" color="secondary" onClick={addTokenToWallet("PAL", PAL_ADDRESS)}>
                        <SvgIcon
                      ...
                      <Button variant="contained" color="secondary" onClick={addTokenToWallet("sPAL", SPAL_ADDRESS)}>
                        <SvgIcon
